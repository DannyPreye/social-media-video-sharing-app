...
import { BsPlay, BsPlayFill, BsFillPauseFill } from 'react-icons/bs'
...
const VideoCard: NextPage<IProps> = ({ post }) => {
    ...
    return (
        <div className='flex flex-col border-b-2 border-gray-200 padding-b-6 '>
            <div >
                ...
                <div className="rounded-3xl" onMouseEnter={() => setIsHover(true)} onMouseLeave={() => setIsHover(false)}>
                    ...
                    {isHover && (
                        <div className='absolute bottom-6  cursor-pointer left-8 md:left-14 lg:left-0  flex gap-10 lg:justify-between w-[100px] md:w-[50px] p-3'>
                            {playing ? <button onClick={onVideoPress}><BsFillPauseFill className='text-black text-2xl lg:text-4xl' /></button > : <button onClick={onVideoPress}><BsPlayFill className='text-black text-2xl lg:text-4xl' /></button>}
                            {isVideoMuted ? <button onClick={() => setIsVideoMuted(false)}><HiVolumeOff className='text-black text-2xl lg:text-4xl' /></button> : <button onClick={() => setIsVideoMuted(true)}><HiVolumeUp className='text-black text-2xl lg:text-4xl' /></button>}
